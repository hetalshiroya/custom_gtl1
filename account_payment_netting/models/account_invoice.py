...
class AccountInvoice(models.Model):
    ...
    def _compute_unpaid_move_lines(self):
        for inv in self:
            inv.unpaid_move_lines = inv.move_id.line_ids.filtered(lambda r: not r.reconciled and r.account_id.internal_type in ('payable', 'receivable'))
    ...
    @api.model
    def _get_netting_move_lines(self, payment_line, partner, debtors, total_debtors, creditors, total_creditors):
        netting_amount = min(total_creditors, total_debtors)
        field_map = {1: 'debit', 0: 'credit'}
        move_lines = []
        for i, group in enumerate([debtors, creditors]):
            print ("I>>>>I", i)
            print ("Igroup   I", group)
    ...
    def register_payment(self, payment_line, writeoff_acc_id=False, writeoff_journal_id=False):
        ...
        line_to_netting = self.mapped('unpaid_move_lines').ids
        line_to_netting = self.env['account.move.line'].search([('id', 'in', line_to_netting)], order='move_id desc')
        ...
        (debtors, total_debtors, creditors, total_creditors) = self._get_netting_groups(account_groups)
        # Create move lines
        move_lines = self._get_netting_move_lines(payment_line, line_to_netting[0].partner_id, debtors, total_debtors, creditors, total_creditors)
        if move_lines:
        ...
        for move_line in payment_move.line_ids:
            if move_line == payment_line:  # Keep this for super()
                continue
            to_reconcile = move_line + line_to_netting.filtered(lambda x: x.account_id == move_line.account_id)
            to_reconcile.filtered('account_id.reconcile').filtered(lambda r: not r.reconciled).reconcile()
        return super().register_payment(payment_line.filtered(lambda l: not l.reconciled), writeoff_acc_id=writeoff_acc_id, writeoff_journal_id=writeoff_journal_id)
