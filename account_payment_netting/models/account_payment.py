...
class AccountAbstractPayment(models.AbstractModel):
    ...
    netting = fields.Boolean(string='Netting', help="Technical field, as user select invoice that are both AR and AP")
    register_payment = fields.Boolean(string='Register Payment?', default=False, help='The balance of the Contra will create Payment automatically')
    ...
    def default_get(self, fields):
        ...
        if ap and ar:  # Both AP and AR -> Netting
            rec.update({'netting': True,
                        'multi': True,  # With netting, allow edit amount
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _validate_invoice_netting(self, invoices):
        ...
        if paid_invoices:
            raise UserError(_('Some selected invoices are already paid: %s') % paid_invoices.mapped('number'))
