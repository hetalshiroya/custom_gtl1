...
class ReportDynamicFinancial(models.AbstractModel):
    ...
    def get_account_lines(self, data):
        _logger.warning('get_account_lines')
        lines = []
        ...
        _logger.warning('get_account_lines 2')
        for report in child_reports:
            ...
            if report.display_detail == 'no_detail':
                ...
                continue
            _logger.warning('get_account_lines 3')
            if res[report.id].get('account'):
                ...
                _logger.warning('get_account_lines 4')
        return lines

    @api.model
    def get_report_values(self, docids, data=None):
        _logger.warning('get_report_values')
        data = dict({'form': data})
        ...
        if data['form'].get('date_filter'):
            ...
            #data['form']['date_from'] = datetime.strptime(date_from, '%Y-%m-%d').strftime(date_format)
            #data['form']['date_to'] = datetime.strptime(date_to, '%Y-%m-%d').strftime(date_format)
            _logger.warning('date from:' + date_from)
            _logger.warning('date to:' + date_to)
            #_logger.warning('date from 2:' + str(datetime.strptime(date_from, '%Y-%m-%d').strftime(date_format)))
     ...
     #       _logger.warning('date from:' + str(datetime.strptime(data['form'].get('date_from', False), date_format)))
            #used_context['date_from'] = datetime.strptime(data['form'].get('date_from', False), date_format).strftime(date_format)
            #used_context['date_to'] = datetime.strptime(data['form'].get('date_to', False), date_format).strftime(date_format)
        ...
        _logger.warning('get_report_values 2')
        comparison_context = {}
        ...
        if data['form'].get('filter_cmp') == 'filter_date':
            ...
            if data['form'].get('date_to_cmp', ''):
                comparison_context['date_to'] = datetime.strptime(data['form'].get('date_to_cmp', ''), date_format).strftime('%Y-%m-%d')
            if data['form'].get('date_from_cmp', ''):
                comparison_context['date_from'] = datetime.strptime(data['form'].get('date_from_cmp', ''), date_format).strftime('%Y-%m-%d')
        ...
        _logger.warning('get_report_values 3')
        report_lines = self.get_account_lines(data.get('form'))
        _logger.warning('End get_account_lines')
    ...
    def action_pdf(self, docids, data=None):
        _logger.warning('action_pdf')
        data, report_lines = self.get_report_values(docids, data)

        langs = self.env['res.lang'].search([('active', '=', True)])[0]
        if langs:
            date_format = langs.date_format
            if data['form']['date_from_cmp']:
                data['form']['date_from_cmp'] = datetime.strptime(data['form'].get('date_from_cmp', ''),
                                                                  date_format).strftime('%Y-%m-%d')
            if data['form']['date_to_cmp']:
                data['form']['date_to_cmp'] = datetime.strptime(data['form'].get('date_to_cmp', ''),
                                                                  date_format).strftime('%Y-%m-%d')
            if not data['form'].get('date_filter'):

                if data['form']['date_from']:
                    data['form']['date_from'] = datetime.strptime(data['form'].get('date_from', ''),
                                                                      date_format).strftime('%Y-%m-%d')
                if data['form']['date_to']:
                    data['form']['date_to'] = datetime.strptime(data['form'].get('date_to', ''),
                                                                      date_format).strftime('%Y-%m-%d')
    ...
    def action_xlsx(self, docids, data=None):
        _logger.warning('action_xlsx')
        data, report_lines = self.get_report_values(docids, data)
        _logger.warning('End get_report_values')
        return {
            'doc_model': 'accounting.report',
            'data': data['form'],
            'lines': report_lines,
