...
class AccountInvoice(models.Model):
    ...
    # TS - add default sequence
    # @api.model
    # def create(self, vals):
    #     print('create debit note')
    #     if not vals.get('name') and vals.get('type') == 'in_invoice' and vals.get('debit_invoice_id') != False\
    #             and vals.get('journal_id.type') == 'purchase':
    #         print('create debit note 2 if')
    #         if 'company_id' in vals:
    #             vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
    #                 '') or _('New')
    #             print('create debit note 3 if')
    #         else:
    #             vals['name'] = self.env['ir.sequence'].next_by_code('vcn') or _('New')
    #
