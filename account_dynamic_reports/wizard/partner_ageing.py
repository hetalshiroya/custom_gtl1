...
class InsPartnerAgeing(models.TransientModel):
    ...
    def process_detailed_data(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if partner:
                    ...
                    if period_dict[period].get('start') and period_dict[period].get('stop'):
                        SELECT += """ CASE 
                                        WHEN 
                                            ...
                                            COALESCE(l.date_maturity,l.date) <= '%s'
                                        THEN
                                            sum(l.amount_residual_currency)
                                        ELSE
                                            0
                                        END AS %s,""" % (period_dict[period].get('stop'),
                                                         period_dict[period].get('start'),
                        ...
                        SELECT += """ CASE 
                                        ...
                                        ELSE
                                            0
                                        END AS %s,""" % (
                        period_dict[period].get('stop'), 'range_' + str(period))
                    else:
                        SELECT += """ CASE
                                        WHEN
                                            COALESCE(l.date_maturity,l.date) <= '%s' 
                                        ...
                                        ELSE
                                            0
                                        END AS %s """ % (
                        period_dict[period].get('start'), 'range_' + str(period))
            ...
            last_section =  """
                    GROUP BY
                        l.date, l.date_maturity, m.id, m.name, j.name, a.name, cc.id, l.currency_id, reference
                    OFFSET %s ROWS
                    FETCH FIRST %s ROWS ONLY
                """%(offset, fetch_range)
            if self.currency_id:
                sql += " AND l.currency_id=%s"%(self.currency_id.id)
            self.env.cr.execute(SELECT + sql%(type, partner, as_on_date, company_id.id) + last_section)
    ...
    def process_data(self):
        ...
        for partner in partner_ids:
            ...
            if self.currency_id:
                sql += " AND l.currency_id = %s"%(self.currency_id.id)
            self.env.cr.execute(sql%(type, partner.id, as_on_date, company_id.id))
            ...
            if count:
                for period in period_dict:
                    ...
                    sql = """
                        SELECT
                            ...
                            sum(COALESCE(l.amount_residual_currency, 0)) AS balance_currency
                        FROM
                            account_move_line AS l
                        LEFT JOIN
                            account_move AS m ON m.id = l.move_id
                    ...
                    if self.currency_id:
                        sql += " AND l.currency_id = %s" % (self.currency_id.id)
                    amount = 0.0
                    self.env.cr.execute(sql%(as_on_date, as_on_date, type, company_id.id) + where)
                    ...
                    if not fetch_dict[0].get('balance'):
                        ...
                        if not self.currency_id:
                            ...
                            amount = fetch_dict[0]['balance_currency']
                ...
                partner_dict[partner.id].update({'currency_id': self.currency_id and self.currency_id.id})
                partner_dict['Total'].update({'company_currency_id': company_currency_id})
                partner_dict['Total'].update({'currency_id': self.currency_id and self.currency_id.id})
            else:
