    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if data.get('invoice_number', False):
                WHERE += " AND inv.number ILIKE '%%%s%%'"%(data.get('invoice_number', False))
        ...
        for aml in self.env['account.move.line'].browse(aml_lines):
            ...
            for debit_line in aml.matched_debit_ids:
                matched_lines = {
                    ...
                    'description': debit_line.credit_move_id.invoice_id.reference or
            ...
            for credit_line in aml.matched_credit_ids:
                matched_lines = {
                    ...
                    'description': credit_line.debit_move_id.invoice_id.reference or
    ...
    def process_data(self):
        ...
        sql = ('''
            SELECT
                ...
                COALESCE(inv.residual_company_signed,0) AS amount_currency,
        ...
        if self.validate_data():
            ...
            if call_from:
                for line in account_lines:
                    sub_lines = self.build_detailed_move_lines(invoice=line['invid'])
