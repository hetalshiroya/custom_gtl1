...
class GoogleDriveAttachments(models.Model):

    _name = 'google.drive.attachments'
    ...
    tallysheet_id = fields.Many2one('warehouse.tally.sheet')
    sequence = fields.Integer(string="sequence")

    @api.model
    def create(self, vals):
        #print ("AAAAAAA", self._context)
        res = super(GoogleDriveAttachments, self).create(vals)
        if res.file_url and res.file:
            self.env['google.file.upload'].delete_from_google_drive(
                res.file_id)
        if res.tallysheet_id:
            active_model = res.tallysheet_id._name
            active_id = res.tallysheet_id.id
        ...
        if res.stock_picking_id:
            active_model = res.stock_picking_id._name
            active_id = res.stock_picking_id.id
            # print ("Heeeeeeeeeeeeee", active_model, active_id, res.invoice_id)
            folder = self.env['ir.config_parameter'].sudo(
            ).get_param('folder_type')
            parent_id = self.env['ir.config_parameter'].sudo(
    ...
    def unlink(self):
        for rec in self:
            if rec.file_id:
                self.env['google.file.upload'].delete_from_google_drive(
                    rec.file_id)
        return super(GoogleDriveAttachments, self).unlink()
