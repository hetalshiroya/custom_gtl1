...
class ExcelExportTrial(models.TransientModel):
    ...
    file = fields.Binary("Click On Download Link To Download Xls File",
                         readonly = True)
    name = fields.Char("Name" , size = 32, default = 'Trial_Balance.xls')
...
class AccountBalanceReport(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        return self.env.ref('scs_account_report.action_report_trial_balance'
    ...
    def get_trial_data(self):
        ...
        if acc_data:
            acc_data = acc_data[0]
            result['journal_ids'] = 'journal_ids' in acc_data and \
                                    acc_data.get('journal_ids') or False
            result['state'] = 'target_move' in acc_data and \
                                acc_data.get('target_move') or ''
            result['date_from'] = acc_data.get('date_from') or False
            result['date_to'] = acc_data.get('date_to') or False
            result['strict_range'] = True if result['date_from'] else False

            is_ytd = 'is_ytd' in acc_data and \
                                    acc_data.get('is_ytd') or False
        ...
        header1 = xlwt.easyxf('pattern: pattern solid, fore_colour white; \
        borders: top double, bottom double, right thin, bottom_color black; font: bold on, \
        ...
        row = 5
        account_balance_inherit_obj = self.env['report.scs_account_report.report_trialbalance']
        ...
        for acc in account_data:
            ...
            if acc['debit']:
                worksheet.write(row, 2, format(acc['debit'], '.3f')[:-1] , style)
            else:
                worksheet.write(row, 2, base_amt, style)
            if acc['credit']:
                worksheet.write(row, 3, format(acc['credit'], '.3f')[:-1] , style)
            else:
                worksheet.write(row, 3, base_amt, style)
            if acc['balance']:
                worksheet.write(row, 4, format(acc['balance'], '.3f')[:-1] , style)
            else:
            ...
            if is_ytd:
                ...
                if acc['ytd_debit']:
                    worksheet.write(row, 6, format(acc['ytd_debit'], '.3f')[:-1] , style)
                else:
                    worksheet.write(row, 6, base_amt, style)
                if acc['ytd_credit']:
                    worksheet.write(row, 7, format(acc['ytd_credit'], '.3f')[:-1] , style)
                else:
                    worksheet.write(row, 7, base_amt, style)
                if acc['ytd_balance']:
                    worksheet.write(row, 8, format(acc['ytd_balance'], '.3f')[:-1] , style)
                else:
        ...
        if is_ytd:
            ...
            if tot_ytd_deb:
                worksheet.write(row, 6, format(tot_ytd_deb, '.3f')[:-1] , header1)
            else:
                worksheet.write(row, 6, base_amt, header1)
            if tot_ytd_cre:
                worksheet.write(row, 7, format(tot_ytd_cre, '.3f')[:-1] , header1)
            else:
                worksheet.write(row, 7, base_amt, header1)
            if ttl_ytd_balance:
                worksheet.write(row, 8, format(ttl_ytd_balance, '.3f')[:-1] , header1)
            else:
        ...
        module_rec = self.env['excel.export.trial'].create({'file':res,
