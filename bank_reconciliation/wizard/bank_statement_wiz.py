...
class BankStatement(models.Model):
    ...
    def _compute_amount(self):
        ...
        current_update += sum([line.debit - line.credit if line.recon_tick else 0 for line in self.statement_lines])
...
class AccountMoveLine(models.Model):
    ...
    recon_tick = fields.Boolean(string='Recon')

    @api.onchange('recon_tick')
    def _on_tick_change(self):
        if self.recon_tick:
            if self.recon_tick == True:
