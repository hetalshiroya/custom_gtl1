...
class InsPartnerPaymentXlsx(models.AbstractModel):
    _name = 'report.dynamic_xlsx.ins_partner_payment_xlsx'
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if acc_lines:
            for line in acc_lines:
                self.row_pos += 1
                self.sheet.write_datetime(self.row_pos, 0, self.convert_to_date(line.get('payment_date')), self.line_header_date)
                ...
                sub_lines = self.record.build_detailed_move_lines(payment=line.get('payid'),
                                                                  internal_type=line.get('internal_type'), fetch_range=1000000)
                self.row_pos += 1
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet = workbook.add_worksheet('Partner Payments')
        ...
        if record:
            ...
            self.sheet.merge_range(0, 0, 0, 8, 'Partner Payments'+' - '+data[0]['company_id'][1], self.format_title)
