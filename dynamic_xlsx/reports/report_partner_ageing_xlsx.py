...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            # Currency
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Currency'),
                                      self.format_header)
            self.sheet_2.write_string(self.row_pos_2, 1, filter.get('currency') if filter.get('currency') else '',
                                      self.content_header)
    ...
    def prepare_report_contents(self, data, period_dict, period_list, ageing_lines, filter):
        ...
        if ageing_lines:
            for line in ageing_lines:
                ...
                self.row_pos += 1
                if line != 'Total':
                    self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, ageing_lines[line].get('partner_name'), self.line_header)
                else:
                ...
                if self.record.include_details:
                    if line != 'Total':
                        ...
                        for sub_line in sub_lines:
                            ...
                            self.sheet.write_string(self.row_pos, 1, sub_line.get('reference') or '',
                                                    self.line_header_light)
