...
TREE_TEMPLATE = '<table style="width: 100%%; border: none;" title="Conversation">' \
                '<tbody>' \
                '<tr>' \
                '<td style="width: 1%%;"><img class="img-circle" style="height: auto; width: 64px; padding:10px;" src="data:image/png;base64, %s" alt="Avatar" title="%s" width="100" border="0" /></td>' \
                '<td style="width: 99%%;">' \
    ...
    def _subject_display(self):
        ...
        for rec in self:
            ...
            for participant in rec.partner_ids:
                participant_text = "%s %s" % (participant_text,
                                              '<img class="rounded-circle" style="width:24px;max-height:24px;margin:2px;" title="%s" src="data:image/png;base64, %s"/>' % (
                                                  ...
                                                  participant.image_small.decode(
                                                      'utf-8') if participant.image_small else IMAGE_PLACEHOLDER))
            ...
            rec.subject_display = TREE_TEMPLATE % (
                rec.author_id.image_small.decode('utf-8') if rec.author_id and rec.author_id.image_small else IMAGE_PLACEHOLDER,
